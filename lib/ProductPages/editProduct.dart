...
import 'package:destock/ProductPages/editProductImageSlider.dart';
...
class _editProductState extends State<editProduct> {
  ...
  GlobalKey<textSwitchSliderState> discloseSwitchKey = new GlobalKey();
  ...
  Widget buildUpdateButton(BuildContext context) {
    return GestureDetector(
      onTap: () async {
        ...
        Map latLongs = {"lat": location.latitude, "long": location.longitude};
        ...
        for (var image in imageList) {
          if (image['type'] == 'network') {
            imageStrings.add(image['path']); //if the image is already on the server
          } else {
            imageFile = new File(image['path'].toString()); //if the image is newly added from the storage
        ...
        availablity = availSwitchKey.currentState.isOn;
        disclosePrice = discloseSwitchKey.currentState.isOn;
        // log("Switch state: "+availabilityString);
        ...
        var resp = await http.post(url, body: {
          ...
          'price': productPrice,
          'disclosePrice': disclosePrice.toString(),
          'available': availablity.toString(),
        ...
        if (respJson["Status"] == "Success")
          Navigator.of(context).pushReplacement(
              MaterialPageRoute(builder: (context) => productPageSeller()));
        else
        ...
        child: Padding(
          ...
          child: Column(
            children: <Widget>[
              ...
              //Switch > yes / no
              SizedBox(height: 10,),
              textSwitchSlider(onText: 'Yes', offText: 'No', key: discloseSwitchKey),
            ],
          ),
        ));
  }
            ...
            child: Padding(
              ...
              child: Column(
                children: <Widget>[
                  ...
                  // buildSwitch('Avaiable', 'Out of Stock'),
                  textSwitchSlider(onText: 'Availability', offText: 'Out of stock', key: availSwitchKey,),
                  SizedBox(
                    height: 30.0,
                  ),
                  Row(
  ...
  Row buildSwitch(String onText, String offText) {
    return Row(
                  children: [
                    Container(
                      //width: 220,
