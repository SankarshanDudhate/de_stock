...
import 'package:destock/widgets/HomeSeller.dart';
...
class _SignupState extends State<Signup> {
  ...
  void saveData() async {
    ...
    String signupData = await sharedPreferences.getString('signupData');
    var dataJson = jsonDecode(signupData);
    String firebaseToken = sharedPreferences.getString("firebaseDeviceToken");
    // print(signupData);

    // TODO send firebase token
    var url = '$localhostAddress/signup/'; //replace '192.168.43.101' with your ip adrress
    ...
    if(resp["Status"] == "Success") {
      ...
      // }
      Map userData = resp['Data']; // jsonDecode(resp['Data']);
      sharedPreferences.setString("user_id", userData['user_id'].toString());
      sharedPreferences.setString("shareableKey", userData['shareableKey']);
      sharedPreferences.setString("email", userData['email']);
      sharedPreferences.setString("name", userData['name']);
      sharedPreferences.setString("phoneNo", userData['phoneNo']);
      sharedPreferences.setString("firebaseDeviceToken", firebaseToken);
