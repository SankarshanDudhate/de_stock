...
//TODO make search bar work on local products (or can go to backend, just include an extra user_id filter)
...
class _manageProductsState extends State<manageProducts> {
  ...
  Future<List<dynamic>> fetchProducts(bool activeProds) async {
    ...
    String url = localhostAddress+'/users/'+userId+'/products/';
    ...
    if(activeProdList.isEmpty && inactiveProdList.isEmpty) {
      ...
      if(activeProds) activeProdList = [];
      else inactiveProdList = [];
  ...
  Widget productCard(cardData) {
    // print(cardData['enquiries'].runtimeType);
    int enquiryLength = 0;
    if(cardData['enquiries'] != null) {
      enquiryLength = List.from(jsonDecode(cardData['enquiries'])).length;
    }
    return product_card(
      ...
      enquiries: enquiryLength.toString(),
      expiry_date: cardData["expiryDate"].toString(),
      image: cardData['image'], //change image
    );
  }
